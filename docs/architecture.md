...
CLI runTurn
  → runAgentTurn (loop.ts)
      → PluginDriver.run
          ...
          verify     根据 TurnExecution 与实际副作用决定验证；DAG 成功使用图内 verify
...
- DAG Worker / Merge 通过 `session.createChild()` 创建隔离的子 Session，继承当前 cwd、取消信号、已加载 Skill 和当前执行 Provider
...
- `finishTool`：写 tool message + 通知 EventBus；Cursor 可分别调用 `recordAssistant` / `recordToolResult`，保证 transcript 仍按 assistant tool-call → tool result → final answer 排列
- ToT 内部计划、复盘和收尾提示是单轮 note，下轮会清理
...
| `hypotheses` | `string[]` | 单轮 | Planner `llmPlan` / `llmReplan` → `applyHypotheses` | 当前待验证假设；每次整表替换，不是追加 |
| `rejected` | `string[]` | 单轮 | 复盘或换思路时 → `rejectHypotheses` | 本任务内已证伪或主动放弃的方向；去重追加 |
| `confidence` | `number` | 单轮 | 复盘 `updateStateFromRun`；换思路 `llmReplan` 时 -0.15 | 对当前方向的置信度，0–1；`llmReplan` 会下调 |
| `noProgress` | `number` | 单轮 | `noteProgress` 每次尝试自增/归零 | 连续无进展的尝试数；事实、探索轨迹或本轮 operations 任一增长则归零；ToT 重试时 `≥2` 触发 `llmReplan` |
...
映射在 `SESSION_TOOL_TRACKS`（会话级）与 `TURN_TOOL_TRACKS`（单轮级）中集中定义，并兼容 Cursor 常见工具名与 `filePath` / `cmd` 等字段别名。拿不到具体路径时保留工具名占位，确保副作用不会逃过验证。工具开始执行前就记录调用，因此失败的写入/命令尝试也能进入收尾验证。
...
DAG 在执行前校验两条流程约束：edit 必须依赖 explore，并且 edit 后必须有 verify。同批 ready 节点中只有只读 `explore` 并发；`edit`、`verify`、`merge` 串行执行，以免多个 Worker 同时修改共享工作区，或验证/汇总读到变化中的状态。
...
环境变量 `AGENT_PROVIDER=openai|cursor`（见 `packages/platform`）决定默认 backend。主 Agent、DAG Worker 和无工具的 Merge Agent 都使用该 backend；DAG 的 `explore` Worker 只拿只读工具，Cursor 对应使用 SDK 的 `plan` 模式，修改型节点不会并发。

### 结构化 LLM 层（固定 OpenAI）

隐式 Router、Planner、Verify gate、DAG 规划等结构化调用，统一走 `structured-llm-caller.ts` → 模块级 **`openAiLlm`**（`OpenAiLlmProvider` 单例），由 AI SDK `Output.object()` 和 Zod 4 校验输出。用户显式要求 `/dag`、使用 ToT、使用 ReAct 时直接路由，不额外调用模型；仅讨论这些模式仍交给 Router 判断。纯读/无命令 Turn 也不会调用验证门禁模型。
...
`verifyPlugin` 消费结构化的 `TurnExecution`。写入/删除必验，只有执行命令但没有文件变化时才询问 LLM 门禁；无副作用直接结束。DAG 成功依赖图内 verify，DAG 失败则根据已回写的 operations 决定是否补验。
...
| `TurnExecution` | mode 执行结果（mode, succeeded, reason/error, verification），由 verify 消费并返回调用方 |
