...
| `operations` | `TurnOperations` | 视场景 | DAG `Blackboard.mergeFrom` | DAG 合并各 Worker 的写入、删除和命令记录 |
...
通过 `session.ledger.refreshOperations()` 读取，用于 `TurnSummary` 与 verify。
...
`noProgress` 与 `BaseMemory.operations` **不投影**给 LLM，仅供程序侧（Planner 上下文、ToT 重试判断、DAG 合并）使用。
