...
import type {TurnContext, TurnOperations, VerifyGate} from '../session-types.js';
import {formatUserRequest} from '../prompt.js';
import type {VerifyResult} from './types.js';

export function formatTurnContextForGate(context: TurnContext): string {
  return joinSections(
    formatUserRequest(context.userInput),
    formatOperationSection('write_file 写入的文件：', context.operations.writtenFiles),
    formatOperationSection('delete_file 删除的文件：', context.operations.deletedFiles),
    formatOperationSection('run_cmd 执行的命令：', context.operations.executedCommands),
    `Agent 最终回答：\n${context.assistantText || '（无文本回答）'}`
  );
}

export function fallbackVerifyGate(context: TurnContext): VerifyGate {
